...
function insertImg(dist: Element, stat: SecStatus) {
  ...
  if (!dist.querySelector(".encrypt-result")) {
    if (stat.encrypt.bool) {
      ...
      encryptImgEle.setAttribute("title", `このメールは次のアルゴリズムで暗号化されて届きました\n${stat.encrypt.description}`);
    } else {
